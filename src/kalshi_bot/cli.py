...
async def cmd_backtest(args, settings):
    ...
    if not args.offline:
        ...
        try:
            ...
            await eng.close()
    rep = run_backtest(storage, since_days=float(args.days), since_model_change=not args.all)
    storage.close()
    out = rep.to_dict()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--yes", action="store_true")
    s = sub.add_parser("backtest", help="score logged decisions against settlements (since the current model version unless --all)")
    s.add_argument("--days", default=30)
    s.add_argument("--offline", action="store_true")
    s.add_argument("--all", action="store_true", help="include decisions made by earlier model versions")
